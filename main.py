...
COM_PORT = 'COM4'       # Set your Arduino COM port here (e.g., 'COM3' or '/dev/ttyUSB0')
BAUD_RATE = 9600
# ==========================================
...
class ObjectDetectionApp:
    ...
    def update_frame(self):
        ...
        detected_name = "None"
        max_area_found = 200 # Minimum threshold
        ...
        for name, spec in self.color_ranges.items():
            ...
            for cnt in contours:
                ...
                if area > 200:  
                    ...
                    # CRITICAL FIX: Only update the overall status for the LARGEST object found
                    # This prevents small background noise from overriding the real target.
    ...
    def on_closing(self):
        if self.cap.isOpened():
            self.cap.release()
        if self.ser and self.ser.is_open:
            self.ser.write(b"000\n") # Tell Arduino to shut off immediately
            self.ser.close()
        self.window.destroy()
